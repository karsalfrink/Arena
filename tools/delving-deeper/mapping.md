...
| Dragons, Black | Very Young Black Dragon ... Very Old Black Dragon (6 rows) | 1d4 | 5/4/3/2 | 6 | 2..12 | 60 | -/H | 1 | 1d6/1d6+2/2d6 | C | B | D | Flight (12/18/24 by age), Detection (6), AcidBreath, AcidImmunity, Fearlessness from Sub-Adult up, Fear (2) from Old up | Line of acid -> AcidBreath; invulnerable to acid -> AcidImmunity. All dragons: Detection (6), Fearlessness from Sub-Adult (DD adult), Fear (2) from Old (see mapping.md). Unmapped: Swims and breathes underwater indefinitely (no rate given). |
| Dragons, Blue | Very Young Blue Dragon ... Very Old Blue Dragon (6 rows) | 1d4 | 5/4/3/2 | 6 | 2+2..14 | 60 | -/H | 1 | 1d6/1d6+2/2d6 | C | B | D | Flight (12/18/24 by age), Detection (6), VoltBreath, VoltImmunity, Fearlessness from Sub-Adult up, Fear (2) from Old up | Line of lightning -> VoltBreath; invulnerable to lightning -> VoltImmunity. Unmapped: Buries itself in sand for ambush. |
| Dragons, Golden | Very Young Gold Dragon ... Very Old Gold Dragon (6 rows) | 1d4 | 5/4/3/2 | 6 | 2-1..16 | 60 | -/H | 1 | 1d6/1d6+2/2d6/3d6 | L | B | D | Flight (12/18/24 by age), Detection (6), FireBreath, SaveBonus (4), Spells, Fearlessness from Sub-Adult up, Fear (2) from Old up | Named 'Gold Dragon' (Arena's spelling) so Monster.java's Gold Dragon spell handling applies: bare 'Spells' makes Arena add two spells per level up to the age category, which tracks DD's 'as many spells as hit dice, up to 3rd level (6th for ancient goldens)' closely enough. Saves +4 vs magic -> SaveBonus (4). Sonic breath has no SpecialType: FireBreath used as a plain-damage stand-in (see NOTES). Unmapped: Breath is sound, not fire (also damages structures). Cannot be subdued; polymorphs into human form. |
| Dragons, Green | Very Young Green Dragon ... Very Old Green Dragon (6 rows) | 1d4 | 5/4/3/2 | 6 | 2+1..13 | 60 | -/H | 1 | 1d6/1d6+2/2d6 | C | B | D | Flight (12/18/24 by age), Detection (6), PoisonBreath, Fearlessness from Sub-Adult up, Fear (2) from Old up | Cone of chlorine -> PoisonBreath. Unmapped: Immune to poison (no PoisonImmunity in SpecialType). |
| Dragons, Red | Very Young Red Dragon ... Very Old Red Dragon (6 rows) | 1d4 | 5/4/3/2 | 6 | 2+3..15 | 60 | -/H | 1 | 1d6/1d6+2/2d6/3d6 | C | B | D | Flight (12/18/24 by age), Detection (6), FireBreath, FireImmunity, Fearlessness from Sub-Adult up, Fear (2) from Old up | Cone of fire -> FireBreath; invulnerable to fire -> FireImmunity. |
| Dragons, White | Very Young White Dragon ... Very Old White Dragon (6 rows) | 1d4 | 5/4/3/2 | 6 | 2-1..11 | 60 | -/H | 1 | 1d6/1d6+2/2d6 | C | B | D | Flight (12/18/24 by age), Detection (6), ColdBreath, ColdImmunity, Fearlessness from Sub-Adult up, Fear (2) from Old up | Cone of cold -> ColdBreath; invulnerable to cold -> ColdImmunity. Unmapped: Swims and tunnels through snow and ice (no rates given). |
...
| Elemental, Air | Eight Hit Dice Air Elemental ... Sixteen Hit Dice Air Elemental (3 rows) | 1 | 3 | 36 | 8..16 | - | - | 1 | 1d6+1 | N | X | X | Flight (36), Whirlwind (3), MagicToHit (1) | HD 8/12/16 -> three rows. Move '-/36': flight only, so MV 36 plus Flight (36) as master does. 2-7 damage -> 1d6+1. Whirlwind 3" wide at base -> Whirlwind (3), as master. 'Elementals of any sort are affected by magical weapons only' (Elementals group introduction) -> MagicToHit (1); master uses (2), DD says any magic weapon. Unmapped: +2 to hit vs aerial opponents. Whirlwind height grows with HD. |
| Elemental, Earth | Eight Hit Dice Earth Elemental ... Sixteen Hit Dice Earth Elemental (3 rows) | 1 | 3 | 6 | 8..16 | - | - | 1 | 3d6 | N | X | X | MagicToHit (1) | 3-18 vs opponents on solid ground -> 3d6 (2-12 vs airborne not modelled). Magic weapons only (Elementals introduction) -> MagicToHit (1). Unmapped: Secondary move 6 is through earth; cannot cross water; move earth drives it back for 6-36. |
| Elemental, Fire | Eight Hit Dice Fire Elemental ... Sixteen Hit Dice Fire Elemental (3 rows) | 1 | 3 | 12 | 8..16 | - | - | 1 | 2d6 | N | X | X | MagicToHit (1) | Summary table says HD 11+3 but the Explanation of Monsters gives 8/12/16 for all elementals; 8/12/16 used (see NOTES). 2-12 damage -> 2d6 (2-7 vs fire creatures not modelled). No fire immunity is stated, so none added. Magic weapons only (Elementals introduction) -> MagicToHit (1). Unmapped: Ignites inflammables; cannot cross water. |
| Elemental, Water | Eight Hit Dice Water Elemental ... Sixteen Hit Dice Water Elemental (3 rows) | 1 | 3 | 6 | 8..16 | - | - | 1 | 2d6 | N | X | X | Swimming (18), MagicToHit (1) | 2-12 in water, 1-6 on land -> 2d6 (the in-water figure, as master). Magic weapons only (Elementals introduction) -> MagicToHit (1). Unmapped: Overturns boats; must stay within 6" of water. |
...
| Giants, Hill | Hill Giant | 1d8 | 4 | 12 | 8 | 30 | E | 1 | 2d6 | C | H | D | RockHurling, Fearlessness | Stats from Table 3.10. The Giants group introduction gives every giant rock hurling for 2-12 (RockHurling, Arena's rock is 2d6) and 'need never check morale against man-types' (Fearlessness). |
| Giants, Stone | Stone Giant | 1d8 | 2 | 12 | 9 | 30 | E | 1 | 2d6 | N | H | D | RockHurling, Fearlessness | Hurls rocks for 3-18 (2-12 for other giants) -> RockHurling (Arena's rock does 2d6). Fearlessness per the Giants introduction. |
| Giants, Frost | Frost Giant | 1d8 | 4 | 12 | 10+1 | 30 | E | 1 | 2d6+1 | C | H | D | RockHurling, Fearlessness, ColdImmunity | Invulnerable to cold -> ColdImmunity. 3-13 -> 2d6+1 (as master). RockHurling, Fearlessness per the Giants introduction. |
| Giants, Fire | Fire Giant | 1d8 | 3 | 12 | 11+3 | 30 | E | 1 | 2d6+2 | C | H | D | RockHurling, Fearlessness, FireImmunity | Invulnerable to fire -> FireImmunity. 4-14 -> 2d6+2 (as master). RockHurling, Fearlessness per the Giants introduction. |
| Giants, Cloud | Cloud Giant | 1d8 | 3 | 15 | 12+2 | 30 | E | 1 | 3d6 | N | H | D | RockHurling, Fearlessness, Detection | Keen smell, rarely surprised -> Detection (master's code for the same OD&D text). RockHurling, Fearlessness per the Giants introduction. |
| Giants, Storm | Storm Giant | 1d8 | 2 | 15 | 15 | 30 | E | 1 | 3d6+3 | N | H | D | RockHurling, Fearlessness, VoltImmunity | Hurls rocks -> RockHurling. Invulnerable to lightning -> VoltImmunity. 6-21 -> 3d6+3 (as master). Fearlessness per the Giants introduction. Unmapped: Weather control (master writes WeatherControl). Lightning strike for 8-48 in thunderstorm conditions. |
...
| Golems, Clay | Clay Golem | 1 | 2 | 6 | 12 | - | - | 1 | 2d6 | N | B | D | MagicToHit (1), MagicImmunity | Only harmed by magic weapons -> MagicToHit (1). Name ends in Golem so Arena adds Golem. 'Largely invulnerable to harmful magic' (Golems introduction) -> MagicImmunity, as master; the few spells DD says do affect it are listed below. Unmapped: Hasted for three turns after one turn of combat. Disintegrate slows it; move earth drives it back for 6-36. Its wounds need a 9th-level cleric to cure. |
| Golems, Flesh | Flesh Golem | 1 | 9 | 9 | 9 | - | - | 1 | 2d6 | N | B | D | MagicToHit (1), MagicImmunity | Only harmed by magic weapons -> MagicToHit (1). 'Largely invulnerable to harmful magic' (Golems introduction) -> MagicImmunity, as master. Unmapped: Slowed by cold and fire spells; healed by lightning. |
...
| Horses, Mule | Mule | 1 | 7 | 12 | 2 | - | - | 0 | 0 | N | A | D | - | Does not attack -> Atk 0 / Dam 0. 'Only mules are agile enough to enter' a dungeon -> Env D. |
...
| Living statues, Iron | Iron Living Statue | 1 | 2 | 6 | 18 | - | - | 1 | 4d6 | N | B | D | Golem, MagicToHit (3), MagicImmunity, PoisonBreath (1) | Golem added explicitly (name does not end in Golem) for construct behaviour. Only +3 weapons harm it -> MagicToHit (3). 1" cloud of poison gas -> PoisonBreath (1), master Iron Golem idiom. 'Largely invulnerable to harmful magic' (Living Statues introduction) -> MagicImmunity. Unmapped: Slowed by lightning; healed by fire. |
| Living statues, Stone | Stone Living Statue | 1 | 5 | 6 | 14 | - | - | 1 | 3d6 | N | B | D | Golem, MagicToHit (2), MagicImmunity, Slowing | Golem added explicitly. Slows one target per turn -> Slowing (master Stone Golem idiom). Only +2 weapons harm it -> MagicToHit (2). 'Largely invulnerable to harmful magic' (Living Statues introduction) -> MagicImmunity. Unmapped: Slowed by cold and fire; healed by stone to flesh. |
